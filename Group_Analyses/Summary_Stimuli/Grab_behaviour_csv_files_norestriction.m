cc
destinationFolder ='Z:\Xiaochen\Sniff_Trigger_for_Izumi';
...
for gg = 1:length(grps)
    ...
    condition = strcmp(TT.NumCellGroup,NumCellGroup)  & strcmp(TT.background,bkground); % create a logical array for the condition
    rst = TT(condition,:); % extract the rows that satisfy the condition

    % Process each animal
    for i = 1:height(rst)
        mouseID = rst.Name_Behaviour{i};
        if contains(mouseID,'Tbx_234','IgnoreCase',true)
        else
            ...
            for j = 1:length(csvFiles)
                ...
                currentCsvFile = csvFiles(j).name;
                ...
                fullSourceFilePath = fullfile(currentSubFolder, currentCsvFile);

                % Copy the file to the destination folder
                copyfile(fullSourceFilePath, destinationFolder);
        ...
        %         writetable(outputTable, 'C:\Users\yycxx\OneDrive - OIST\Thesis\Behaviour\Group_Analyses\Summary_Stimuli\Data_FileNames_no_restriction.csv');
    end
end
