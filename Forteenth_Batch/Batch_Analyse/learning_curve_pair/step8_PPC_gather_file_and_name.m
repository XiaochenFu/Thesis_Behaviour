...
for si = 1:length(Stimuli_list)
    ...
    for mi = 1:length(Mouse_list)
        ...
        if exist(light_cat_path)
            %% light part
            copyAndRenameMatFiles_PPC(light_cat_path)
            ...
            %% odour part
            copyAndRenameMatFiles_PPC(odour_cat_path)
    ...
    if exist(Odour_Stim_Raw_FileNames)
        % Define the output directory and csv file path
        outputDir = fullfile(currentDir, 'PPC_Group', StimuliName);
